...
impl MoqServer {
    ...
    #[cfg(feature = "moq")]
    fn moq_origins(&self) -> (moq::OriginProducer, moq::OriginProducer) {
        (
            moq::Origin::random().produce(),
            moq::Origin::random().produce(),
        )
    }
    ...
    async fn run_moq_session(
        &self,
        session: moq::Session,
        outgoing_origin: moq::OriginProducer,
        incoming_origin: moq::OriginConsumer,
...
pub struct MoqSessionRuntime {
    ...
    frame_numbers: crate::voice::RtpFrameNumberMapper,
}
...
impl MoqSessionRuntime {
    ...
    async fn authenticate_client(
        &mut self,
        auth: crate::protocol::AuthRequest,
    ) -> Result<Vec<ServerEvent>, String> {
    ...
    async fn attach_moq(
        ...
        let mut broadcast = moq::Broadcast::new().produce();
        let mut control_down = broadcast.create_track(moq::Track::new(CONTROL_DOWN_TRACK))?;
        let mut catalog = broadcast.create_track(moq::Track::new(CATALOG_TRACK))?;
        ...
        for slot in 0..max_tracks {
            audio_tracks.push((
                slot,
                broadcast.create_track(moq::Track::new(MoqTrackNames::audio_down_slot(slot)))?,
            ));
        }
...
fn audio_target(target: VoiceTargetKind) -> shitspeak_runtime::messages::encoder::AudioTarget {
    match target {
        VoiceTargetKind::Normal => shitspeak_runtime::messages::encoder::AudioTarget::Normal,
        VoiceTargetKind::ServerLoopback => {
            shitspeak_runtime::messages::encoder::AudioTarget::ServerLoopback
        }
        VoiceTargetKind::Slot(slot) => {
            shitspeak_runtime::messages::encoder::AudioTarget::VoiceTarget(slot)
        }
    }
}
...
fn outbound_context(audio: &Audio) -> String {
    match audio.target {
        shitspeak_runtime::messages::encoder::AudioTarget::Normal => "normal".to_string(),
        shitspeak_runtime::messages::encoder::AudioTarget::ServerLoopback => "loopback".to_string(),
        shitspeak_runtime::messages::encoder::AudioTarget::VoiceTarget(slot) => {
            format!("target:{slot}")
        }
    }
...
mod tests {
    ...
    async fn outbound_audio_task_maps_tcp_voice_to_speaker_slot_events_and_hang_frames() {
        ...
        let frame = Audio {
            target: shitspeak_runtime::messages::encoder::AudioTarget::Normal,
    ...
    async fn outbound_audio_track_writes_hang_frame_bytes() {
        let mut track = moq::Track::new("audio/down/slot/0").produce();
