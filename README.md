...
By default, S2S advertises private RFC1918 IPv4 and IPv6 unique-local addresses so LAN, VPN, and container clusters continue to work. Set `s2s.advertise_private_ips = false` when this node should only advertise non-private addresses. Wildcard listeners can also advertise addresses discovered from explicitly allowed local interfaces with `s2s.local_interface_advertise = ["tailscale0"]`.
