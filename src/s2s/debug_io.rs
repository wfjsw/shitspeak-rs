...
use crate::s2s::application::proto as app_proto;
use crate::s2s::overlay::proto::{OverlayBody, OverlayControlBody, OverlayData};
use crate::s2s::replications::proto as repl_proto;

static COUNTERS: LazyLock<Mutex<PacketCounters>> =
    LazyLock::new(|| Mutex::new(PacketCounters::new()));

#[derive(Clone, Debug, Eq, PartialEq)]
pub(crate) struct PacketKind {
    name: String,
}
...
pub(crate) struct PacketIoSnapshot {
    kind: String,
    sent_bytes: u64,
    recv_bytes: u64,
    total_bytes: u64,
...
pub(crate) fn classify_overlay_body(body: &OverlayBody) -> PacketKind {
    match body {
...
pub(crate) fn record_sent(kind: PacketKind, bytes: usize) {
    COUNTERS.lock().record_sent(kind, bytes);
}

pub(crate) fn record_received(kind: PacketKind, bytes: usize) {
...
mod tests {
    ...
    use crate::s2s::overlay::proto::{OverlayData, level_to_wire, node_to_wire};
    use crate::s2s::replications::proto::{StrictBody, StrictClockTick, wrap_strict};
    ...
    fn overlay_data(tag: u32, payload: Bytes) -> OverlayData {
        OverlayData {
            ...
            message_class: crate::s2s::overlay::proto::class_to_wire(MessageClass::Regular),
            payload,
